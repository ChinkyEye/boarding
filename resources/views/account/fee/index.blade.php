...
<section class="content">
  <div class="card">
    ...
    <form role="form" method="POST" action="{{ route('account.teacher-attendance.store')}}">
    @csrf
      <div id="replaceTable">
...
<script type="text/javascript">
  $("body").on("change","#shift_data", function(event){
    ...
    $.ajax({
      type:"POST",
      dataType:"JSON",
      url:"{{route('account.getClassList')}}",
      data:{
        _token: token,
        shift_id: shift_id
      },
...
<script type="text/javascript">
  $("body").on("change","#class_data", function(event){
    ...
    $.ajax({
      type:"POST",
      dataType:"JSON",
      url:"{{route('account.getSectionList')}}",
      data:{
        _token: token,
        class_id: class_id,
        shift_id: shift_id,
...
<script>
  ...
  $('#search').click(function () {
    var searchParams = {};
    var base_url = "{{route('account.getStudentFeeList')}}";
