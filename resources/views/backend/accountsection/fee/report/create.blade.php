  ...
  <table class="table table-bordered table-hover position-relative m-0">
    ...
    @foreach ($fee_list as $key=>$element)
      <tr class="text-center">
        <td>{{$key+1}}</td>
        <td class="text-left">{{$element->getStudentinfo->name}} {{$element->getStudentinfo->middle_name}} {{$element->getStudentinfo->last_name}}({{$element->getUserInfo->student_code}})</td>
