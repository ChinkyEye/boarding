     ...
     @foreach($students as $key=>$student)
      <div class="col-md-6">
        <div class="card">
          ...
          <div class="col">
            <p class="font-weight-bold bitch" ><span class="pl-5">This</span> is to certify that MR/Miss/Mrs. <abbr title="">{{$student->getStudentUser->name}} {{$student->getStudentUser->middle_name}} {{$student->getStudentUser->last_name}}</abbr title=""> son/daughter of <abbr title="">{{$student->Student_has_parent->father_name}}</abbr>, inhabitant of <abbr title="">{{$student->address}}</abbr> VDC/Municipality Morang District Koshi Zone was  bonafide student of this school. He/She has successfully passed the class <abbr title="">{{$student->getClass->name}}</abbr> examination of the year <abbr title="">{{$student->getStudentViaBatch->Batch->name}}</abbr> B.S / A.D. According to the School record his/her date of birth is <abbr title="">{{$student->dob}}</abbr> A.D.He/She bears a good moral character.<br>
