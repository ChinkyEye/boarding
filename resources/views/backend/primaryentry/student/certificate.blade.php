...
<section class="content">
  ...
  <div class="card" id="printTable">
    ...
    <div class="col">
    	<p class="font-weight-bold bitch" ><span class="pl-5">This</span> is to certify that MR/Miss/Mrs. <abbr title="">{{$students->getStudentUser->name}} {{$students->getStudentUser->middle_name}} {{$students->getStudentUser->last_name}}</abbr title=""> son/daughter of <abbr title="">{{$students->Student_has_parent->father_name}}</abbr>, inhabitant of <abbr title="">{{$students->address}}</abbr> VDC/Municipality Morang District Koshi Zone was  bonafide student of this school. He/She has successfully passed the class <abbr title="">{{$students->getClass->name}}</abbr> examination of the year <abbr title="">{{$students->getStudentViaBatch->Batch->name}}</abbr> B.S / A.D. According to the School record his/her date of birth is <abbr title="">{{$students->dob}}</abbr> A.D.He/She bears a good moral character.<br>
