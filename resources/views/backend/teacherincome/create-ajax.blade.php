...
<button type="button" class="btn btn-info btn-xs hidden-print float-right" onclick="PrintDiv('printDiv')" >PRINT<i class="fa fa-print"></i></button>
 --}}
<div id="printDiv">  
  <form role="form" method="POST" action="{{route('admin.salary.store')}}" class="validate" id="validate">
    <div class="card-body">
      @csrf
      <div class="row">
      ...
      <div class="row">
        ...
        {{-- <div class="form-group col-sm-6">
          ...
          <input type="text"  class="form-control max" id="insurance" placeholder="Enter Amount" name="insurance"   autocomplete="off" value="{{ old('insurance') }}" autofocus>
          @error('insurance')
          <span class="text-danger font-italic" role="alert">
            <strong>{{ $message }}</strong>
          </span>
