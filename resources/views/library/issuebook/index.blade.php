...
<section class="content">
  <div class="card">
    <div class="card-header">
      <a href="{{ route('library.issuebook.create')}}" class="btn btn-sm btn-info text-capitalize" data-toggle="tooltip" data-placement="top" title="Issue Book">Add {{ $page }} </a>
        ...
        <table class="table table-bordered table-hover position-relative w-100 m-0">
          ...
          <tbody>
            ...
            <tr class="text-center">
              ...
              <td>
                @if($issuebook->return_date != null)
                <a href="{{route('library.issuebook.show',$issuebook->id)}}" class='btn btn-xs btn-outline-success' data-toggle='tooltip' data-placement='top' title='View Detail'><i class='fa fa-eye'></i></a>
                @else
                <a href="{{route('library.issuebook.edit',$issuebook->id)}}" class='btn btn-xs btn-outline-info' data-toggle='tooltip' data-placement='top' title='Update' data-original-title='Update.'><i class='fas fa-edit'></i></a> 
                <form action="{{route('library.issuebook.destroy',$issuebook->id)}}" method='issuebook' class='d-inline-block delete-c' data-toggle='tooltip' data-placement='top' title='Permanent Delete'>
...
<div class="modal fade" id="modal-default" data-backdrop="static" data-keyboard="false">
  <div class="modal-dialog"  role="document">
    <div class="modal-content">
      ...
      <form role="form" method="POST" action="{{ route('library.issuebook.getReturnDateSave')}}" class="categoryform" id="signup">
        {{ csrf_field() }}
...
<script type="text/javascript">
  $("body").on("click", "#issuebookSearch", function(event){
      ...
      $.ajax({
        type:"POST",
        dataType:"html",
        url: "{{URL::route('library.getIssuebookRecord')}}",
        data: {
          _token: token,
...
<script>
    $('.delete-c').on('click', function (e) {
      ...
      var url = route('library.issuebook.destroy') ;
      var token = $('meta[name="csrf-token"]').attr('content');
      swal({
        title: 'Are you sure?',
