        ...
        <div class="row">
          ...
          @include('main.school.info.report.teacher.nav')
            <div class="tab-content p-3" id="nav-tabContent">
              <form role="form" class="row mb-3" method="GET" action="{{ route('main.school.teacher.report.salary.search',[$school_info->slug])}}">
                <div class="col-md input-group mb-3">
                  <select class="form-control" id="filter_month" name="filter_month">
                    <option value="1" {{ $nepali_month == '1' ? 'selected' : ''}}>Baisakh</option>
                    <option value="2" {{ $nepali_month == '2' ? 'selected' : ''}}>Jestha</option>
                    <option value="3" {{ $nepali_month == '3' ? 'selected' : ''}}>Asar</option>
                    <option value="4" {{ $nepali_month == '4' ? 'selected' : ''}}>Shrawan</option>
                    <option value="5" {{ $nepali_month == '5' ? 'selected' : ''}}>Bhadra</option>
                    <option value="6" {{ $nepali_month == '6' ? 'selected' : ''}}>Ashoj</option>
                    <option value="7" {{ $nepali_month == '7' ? 'selected' : ''}}>Kartik</option>
                    <option value="8" {{ $nepali_month == '8' ? 'selected' : ''}}>Mangsir</option>
                    <option value="9" {{ $nepali_month == '9' ? 'selected' : ''}}>Poush</option>
                    <option value="10" {{ $nepali_month == '10' ? 'selected' : ''}}>Magh</option>
                    <option value="11" {{ $nepali_month == '11' ? 'selected' : ''}}>Falgun</option>
                    <option value="12" {{ $nepali_month == '12' ? 'selected' : ''}}>Chaitra</option>
                  </select>
             ...
             {{--  <form action="{{route('admin.report.teacher.salarylist.export')}}" method="GET" class="d-inline-block">
              ...
              <form action="{{route('admin.report.teacher.salarylist.export')}}" method="GET" class="d-inline-block">
                <input type="hidden" id="excMonth" name="excMonth" value="{{$nepali_month}}">
                <input type="hidden" id="excType" name="excType" value="profile">
...
{{-- <script type="text/javascript">
  $("body").on("change","#filter_month", function(event){
    ...
    $.ajax({
      type:"GET",
      dataType:"html",
      url:"{{route('admin.report.teacher.salary')}}",
      data:{
        _token: token,
