...
<section class="content">
  <button type="button" class="btn btn-info btn-xs rounded-0 hidden-print" onclick="PrintDiv('printDiv')" >PRINT<i class="fa fa-print"></i></button>
  <div class="card card-info">
    <div class="card-body">
      <div id="printDiv">
        <div class="text-center">
          @foreach($settings_school as $key=>$setting)
        ...
        <div class="text-center my-4">
          ...
          <div class="text-capitalize mt-3">
            <u class="h5 font-weight-bold">{{$exam_info->name}}</u>
            <span class="position-absolute mr-4" style="right: 0;"><b>Academic Year:</b> {{$db_student->getSchoolUserStudent->getBatch->name}}</span>
          </div>
        </div>
        <table class="table table-borderless table-hover table-sm">
          <tr>
            ...
            <td>: {{$db_student->name}} {{$db_student->middle_name}} {{$db_student->last_name}} ({{$db_student->getSchoolUserStudent->student_code}})</td>
            <td></td>
            <td></td>
          </tr>
          <tr>
            ...
            <td>
              : {{$db_student->getSchoolUserStudent->getClass->name}}
            </td>
            <td width="200px">
              Roll No / Section
            </td>
            <td>
              : {{$db_student->getSchoolUserStudent->roll_no}} / {{$db_student->getSchoolUserStudent->getSection->name}}
            </td>
          </tr>
        </table>
