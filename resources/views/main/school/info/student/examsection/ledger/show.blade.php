...
<section class="content">
  <div class="card">
    <div class="card-header">
      <form action="{{route('main.studenthasmark.ledger.show',$school_info->slug)}}" method="GET">
        @csrf
        <div class="row">
          <div class="col-md">
      ...
      <div class="w-100  table-responsive" id="printTable">
        ...
        <table class="table table-bordered table-hover position-relative m-0 table-sm">
          ...
          <tr class="text-center" key="{{$key}}">
            <td>{{$data->roll_no}}</td>
            <td class="text-left">
              {{$data->getStudentUser->name}} {{$data->getStudentUser->middle_name}} {{$data->getStudentUser->last_name}}
            ...
            @if ($check->count())
              ...
              @endforeach
            @else
              <td colspan="{{$classmarks->count()}}">Marks not updated.</td>
            @endif
            <?php
            $total = 0;
            ?>
            ...
            <td class="print-0">
              ...
              <a target="_blank" href="{{ route('main.studenthasmark.getMarkSheetPrint',[$school_info->slug,$data->slug,$exam->slug]) }}" class="btn btn-xs btn-outline-danger" data-toggle="tooltip" data-placement="top" title="Full Marksheet Print"><i class="fa fa-print"></i></a>
              <a target="_blank" href="{{ route('main.studenthasmark.getMarkPrint',[$school_info->slug,$data->slug,$exam->slug]) }}" class="btn btn-xs btn-outline-info" data-toggle="tooltip" data-placement="top" title="Print Mark">M</a>
              <a target="_blank" href="{{ route('main.studenthasmark.getGradePrint',[$school_info->slug,$data->slug,$exam->slug]) }}" class="btn btn-xs btn-outline-success" data-toggle="tooltip" data-placement="top" title="Print Grade">G</a>
              <a target="_blank" href="{{ route('main.studenthasmark.getBothPrint',[$school_info->slug,$data->slug,$exam->slug]) }}" class="btn btn-xs btn-outline-primary" data-toggle="tooltip" data-placement="top" title="Print Marks & Grade">MG</a>
            </td>
          @else
...
<script type="text/javascript">
  $(document).ready(function(event){
    ...
    $.ajax({
      type:"POST",
      dataType:"JSON",
      url:"{{route('main.getExamShiftList',$school_info->slug)}}",
      data:{
        _token: token,
        data_id: exam_id
      },
      success: function(response){
        ...
        $.ajax({
          type:"POST",
          dataType:"JSON",
          url:"{{route('main.getExamClassList',$school_info->slug)}}",
          data:{
            _token: token,
            data_id: shift_id,
            exam_id: exam_id
